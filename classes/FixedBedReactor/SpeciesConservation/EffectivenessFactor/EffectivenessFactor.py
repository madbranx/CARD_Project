...
class EffectivenessFactor:
    ...
    def calc_effectiveness_factor(self, T, molar_mass_i, stochiometry_i, concentration_i, diameter_pore, tortuosity_particle, porosity_particle, diameter_particle):
        thiele = self.calc_thiele(T, molar_mass_i, stochiometry_i, concentration_i, diameter_pore, tortuosity_particle, porosity_particle, diameter_particle)
        effectiveness_factor = 3/thiele * ( 1/casADi.tanh(thiele) - 1/thiele )
        return effectiveness_factor

    def calc_thiele(self, T, molar_mass_i, stochiometry_i, concentration_i, diameter_pore, tortuosity_particle, porosity_particle, diameter_particle):
        eff_diff_coff = self.calc_eff_diff_coff(T, molar_mass_i, diameter_pore, tortuosity_particle, porosity_particle)
    ...
    def calc_eff_diff_coff(self, T, molar_mass_i, diameter_pore, tortuosity_particle, porosity_particle):
        # Checked
        knudsen_diff_coff = self.calc_knudsen_diff_coff(T, molar_mass_i, diameter_pore)
        # molar diff coff, wie implementieren??
        molar_diff_coff = 7.1497e-5 # TEMP
        eff_diff_coff = ( tortuosity_particle**2 / porosity_particle * (1/molar_diff_coff + 1/knudsen_diff_coff) )**(-1)
        return eff_diff_coff

    def calc_knudsen_diff_coff(self, T, molar_mass_i, diameter_pore):
        # Checked
